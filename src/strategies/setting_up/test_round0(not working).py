...
STATIC_SYMBOL = 'EMERALD'
DYNAMIC_SYMBOL = 'TOMATO'
...
class ProductTrader:
    ...
    def get_walls(self):
        # Use .keys() and check if they exist first
        bids = self.mkt_buy_orders.keys()
        asks = self.mkt_sell_orders.keys()
        
        bid_wall = max(bids) if bids else None # Best Bid
        ask_wall = min(asks) if asks else None # Best Ask
